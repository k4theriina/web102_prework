...
// reduce takes a nameless function and 0
const totContributions = GAMES_JSON.reduce( (acc, game) => {
    return acc + game.backers;
}, 0)

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `${totContributions.toLocaleString()}`;


// grab the amount raised card, then use reduce() to find the total amount raised
const amtRaisedCard = document.getElementById("total-raised");
...
amtRaisedCard.innerHTML = `${totRaised.toLocaleString()}`;
...
function filterUnfundedOnly() {
    ...
    // console.log("length of listOfNotMet:", listOfNotMet.length);
}
...
const displayStr = 
    `A total of ${totRaised} has been raised for ${GAMES_JSON.length} games. Currently, ${numUnfundedGames} game${numUnfundedGames > 1 ? 's' : ''} remains unfunded. We need your help to fund ${numUnfundedGames > 1 ? 'these' : 'this'} amazing game${numUnfundedGames > 1 ? 's' : ''}!`

// create a new DOM element containing the template string and append it to the description container
let templateStr = document.createElement("p");
...
const topPledge = document.createElement("h1");
